...
public class App {
    public static void main(String[] args) {
        ...
        ProductoEscolar cuadernoPrimaria = new ProductoEscolar("ES-201", "Cuaderno Cuadriculado", 10000.0, 15,
                "primaria");
        ProductoEscolar cuadernoSecundaria = new ProductoEscolar("ES-202", "Cuaderno Argollado", 10000.0, 20,
                "secundaria");

        System.out.println("Producto Oficina (+8% recargo):");
        System.out.println("   " + resma.getNombre() + " | Precio Base: $" + resma.getPrecio() + " -> Precio Final: $"
                + resma.calcularPrecioFinal());

        System.out.println("Producto Escolar (Primaria -10% descuento):");
        System.out.println("   " + cuadernoPrimaria.getNombre() + " | Precio Base: $" + cuadernoPrimaria.getPrecio()
                + " -> Precio Final: $" + cuadernoPrimaria.calcularPrecioFinal());

        System.out.println("Producto Escolar (Secundaria sin descuento):");
        System.out.println("   " + cuadernoSecundaria.getNombre() + " | Precio Base: $" + cuadernoSecundaria.getPrecio()
                + " -> Precio Final: $" + cuadernoSecundaria.calcularPrecioFinal());
        ...
        System.out.println("  -> Pedido 1 [Fecha: " + pedido1.getFecha() + "] | Estado: " + pedido1.getEstado()
                + " | Proveedor: " + pedido1.getProveedor().getNombre());
        System.out.println("  -> Pedido 2 [Fecha: " + pedido2.getFecha() + "] | Estado: " + pedido2.getEstado()
                + " | Proveedor: " + pedido2.getProveedor().getNombre());

        System.out.println("\n--- 5. VALIDACIÓN DE EXCEPCIONES ---");

        try {
            System.out.println("Intentando vender 100 cuadernos (Stock disponible: "
                    + cuadernoPrimaria.getCantidadStock() + ")...");
